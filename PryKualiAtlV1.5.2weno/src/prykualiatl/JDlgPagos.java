  ...
  this.fecharegistros= String.valueOf(contenido[JTblPagos.getSelectedRow()][16].toString());
    ...
    private void JBtnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_JBtnEliminarActionPerformed
        ...
        bd_manager obj = new bd_manager();
                  String sql;
                    ResultSet rs;
        DefaultTableModel modelo = (DefaultTableModel)JTblPagos.getModel();
        int clave=0;
        int row = JTblPagos.getSelectedRow();
        String usuarios = JTblPagos.getValueAt(row,0).toString();
        clave=Integer.parseInt(usuarios);

           sql=("DELETE  FROM tblpago WHERE ClvFolio=" + usuarios + "");

           obj.AbcProye(sql);
           try{

               modelo.removeRow(JTblPagos.getSelectedRow());
           }catch (Exception ex) {
