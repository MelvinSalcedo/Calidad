...
//            {
        ...
        if(JBtnBuscar.isEnabled())
        {
            try
            {
               ...
               String sql;
               ResultSet rs;
               sql=("select NoCuenta,Nombre,Direccion,Colonia,Municipio,Telefono,Rfc,Tarifa  FROM tblcliente WHERE NoCuenta='" + JTxtNoCuenta.getText() + "'");
               ...
               while (rs.next())
               {
                 if(b.equals(rs.getString(1)))
                 {
                    // JTxtNoCliente.setText((String)rs.getString(1));
                     ...
                     JTxtTarifa.setText((String)rs.getString(8));
                     encuentra=true;
                     break;
                 }
    ...
    try
            {
               ...
               String sql;
               ResultSet rs;
               sql=("select ClvFolio FROM tblpago ");
               rs= obj.consultar(sql);
              ...
              while (rs.next())
               {
                  ...
                  this.JTxtFolio.setText(Integer.toString(clavenueva));
            }catch (Exception ex) {
                      JOptionPane.showMessageDialog( null, "ERROR AL INTENTAR ABRIR LA TABLA", "ERROR", JOptionPane.ERROR_MESSAGE);
                  }
}
...
public void ultimafecha()
{
    try
           ...
           String b;
               ...
               String sql;
               ResultSet rs;
               sql=("select Clvfolio,Nocuenta,FechaCubierta FROM tblpago where NoCuenta='"+ this.JTxtNoCuenta.getText()+"'");
               rs= obj.consultar(sql);
           b=JTxtNoCuenta.getText();
             int v;             
               while (rs.next())
               {
                 ...
                 {      
                            JTxtFechaCubierta.setText((String)rs.getString(3));
                 }
               }
            }catch (Exception ex) {
                       JOptionPane.showMessageDialog( null, "ERROR AL INTENTAR ABRIR LA TABLA", "ERROR", JOptionPane.ERROR_MESSAGE);
...
//        totmeses=mesdos-mesuno;
 ...
 if(    jTextField2.getText().equals("GUARDAR"))
 {
        ...
        sql = ("INSERT INTO tblpago (ClvFolio,NoCuenta,FechaCubierta,MesesTranscurridos,FechaDePago,Rezago,PagoCalculado,Recargo,Total,FechaDeRegistro) Values (" + JTxtFolio.getText() + "," + JTxtNoCuenta.getText() + ",'" + JTxtFechaCubierta.getText() + "'," + JTxtMesesTranscurridos.getText() + ",'" + JTxtFechaaPagar.getText() + "'," + JTxtRezagos.getText() + "," + JTxtSubtotal.getText() + "," + JTxtRecargos.getText() + "," + JTxtTotal.getText() + ",'" + jTextField1.getText() + "')");
            obj.AbcProye(sql);
JOptionPane.showMessageDialog( null, "REGISTRO DEL PAGO SE GUARDO CORRECTAMENTE", "GUARDANDO", JOptionPane.INFORMATION_MESSAGE);
 ...
 else
 {
     if(    jTextField2.getText().equals("EDITAR"))
     {
        ...
        sql = ("UPDATE tblpago SET FechaCubierta=" + JTxtFechaCubierta.getText() + ",MesesTranscurridos=" + JTxtMesesTranscurridos.getText() + ",FechaDePago=" + JTxtFechaaPagar.getText() + ",Rezago=" + JTxtRezagos.getText() + ",PagoCalculado=" + JTxtSubtotal.getText() + ",Recargo=" + JTxtRecargos.getText() + ",Total=" + JTxtTotal.getText() + ",FechaDeRegistro=" + jTextField1.getText() + " where ClvFolio=" + JTxtFolio.getText() + "");
            obj.AbcProye(sql);
